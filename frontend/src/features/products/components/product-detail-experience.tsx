...
export function ProductDetailExperience({ product }: { product: Product }) {
  ...
  return <>
    ...
    <section className="product-detail section-pad">
      <Container className="product-detail__grid">
        <div className="product-summary">
          <div className="product-summary__topline"><span className={activeProduct.stockQuantity > 0 ? "stock" : "stock stock--off"}>{activeProduct.stock}</span>{activeProduct.hasDiscount && activeProduct.discountPercent && <span className="discount-badge">{new Intl.NumberFormat("fa-IR").format(activeProduct.discountPercent)}٪ تخفیف</span>}<span>{product.categoryName}</span></div>
    ...
    <section className="product-information section-pad"><Container className="product-information__grid"><div><p className="section-eyebrow">رنگ و نقش</p><h2>جزئیات محصول</h2></div><dl><div><dt>ترکیب رنگ</dt><dd>{product.colors}</dd></div><div><dt>طرح</dt><dd>{product.pattern}</dd></div><div><dt>دسته‌بندی</dt><dd>{product.categoryName}</dd></div></dl></Container></section>
