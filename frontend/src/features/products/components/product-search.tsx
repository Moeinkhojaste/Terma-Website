      ...
      <div className="search-panel">
        ...
        <div id="search-suggestions" className="search-suggestions" role="listbox">
          ...
          {results.map((product, index) => {
            ...
            return (
              <button id={`search-result-${product.id}`} type="button" role="option" aria-selected={index === activeIndex} className={index === activeIndex ? "is-active" : ""} onMouseEnter={() => setActiveIndex(index)} onClick={() => openProduct(product)} key={product.id}>
                <span className="search-result__image"><Image src={product.image} alt="" fill sizes="64px" unoptimized={isUnoptimizedMedia(product.image)} /></span>
                <span><strong>{product.name}</strong><small>{product.capacity} · {product.stock}</small></span>
                {isUnavailable ? <b className="search-result__unavailable">ناموجود</b> : <b>{product.price}</b>}
              </button>
            );
          })}
        </div>
