...
const links = [
  ["/admin", "داشبورد"], ["/admin/products", "محصولات"], ["/admin/categories", "دسته‌بندی‌ها"], ["/admin/orders", "سفارش‌ها"], ["/admin/customers", "مشتریان"],
  ["/admin/promotions", "تخفیف‌ها"], ["/admin/shipping", "ارسال"], ["/admin/content", "محتوای سایت"], ["/admin/messages", "پیام‌ها"], ["/admin/reports", "گزارش‌ها"], ["/admin/settings", "تنظیمات"],
] as const;
...
export function AdminShell({ children, title }: { children: ReactNode; title: string }) {
  ...
  useEffect(() => {
    let active = true;
    getAdminSession().then((value) => active && setSession(value)).catch((error) => {
      if (!active) return;
      if (error instanceof ApiError && error.status === 401) router.replace("/admin/login?reason=expired");
    });
    return () => { active = false; };
  }, [router]);

  async function logout() { await logoutAdmin(); router.replace("/admin/login"); router.refresh(); }

  if (!session) return <main className="admin-loading" role="status">در حال بررسی نشست مدیر…</main>;
