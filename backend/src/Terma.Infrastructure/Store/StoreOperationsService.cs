...
public sealed class StoreOperationsService(TermaDbContext db) : IStoreOperationsService
{
    ...
    { var entity = await db.ProductMedia.SingleOrDefaultAsync(x => x.Id == id, cancellationToken) ?? throw new NotFoundException($"Media '{id}' was not found."); if (request.IsPrimary) await db.ProductMedia.Where(x => x.ProductId == entity.ProductId && x.Id != id).ExecuteUpdateAsync(s => s.SetProperty(x => x.IsPrimary, false), cancellationToken); entity.Update(request.PublicUrl, request.AltText, request.Kind, request.SortOrder, request.IsPrimary); await db.SaveChangesAsync(cancellationToken); return Map(entity); }
